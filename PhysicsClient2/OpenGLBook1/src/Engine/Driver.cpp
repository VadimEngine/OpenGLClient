			...
			while (unprocessedSeconds > secondsPerTick) {
				...
				if (TCP) {
					...
					if (myWindow->leftClick) {
						float toSend[3] = { -1, myWindow->mousePosition.x, myWindow->mousePosition.y };
					...
					//click action
					if (myWindow->leftClick) {
						float toSend[4] = { -2, myWindow->mousePosition.x, myWindow->mousePosition.y , conUDP.userId};
...
/// </summary>
/// <remarks>
///	Move outside this file. Maybe in window or handler. Add :q to quit.
/// </remarks>
bool connectionProtocol() {
	int stage = 0;
